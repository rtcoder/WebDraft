...
export function drawText(
  ...
  context.font = getCanvasFont(state);
  context.textAlign = state.textAlign;
  context.textBaseline = 'top';

  const fontSize = getTextFontSize(state);
  const lineHeight = fontSize * 1.25;
  const lines = value.split('\n');
  const x = getTextLineX(bounds, state);
...
export function getCanvasFont(state: EditorState): string {
  const style = state.textItalic ? 'italic' : 'normal';
  const weight = state.textBold ? '700' : '400';

  return `${style} ${weight} ${getTextFontSize(state)}px ${state.textFontFamily}`;
}

export function getTextFontSize(state: EditorState): number {
  return Math.max(8, state.size * 2);
}
...
function getTextLineX(bounds: SizeWithPosition, state: EditorState): number {
